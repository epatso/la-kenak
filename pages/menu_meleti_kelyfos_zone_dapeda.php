...
<br/>
	<div id="zone_dapeda_table"></div>
	<div id="zone_dapeda_info"></div>
	<script>
	get_zone_dapeda();
	
	//Εμφάνιση πίνακα με όλα τα δάπεδα
	function get_zone_dapeda(page){
	page = typeof page !== 'undefined' ? page : 1;
		document.getElementById('wait').style.display="inline";
		//AJAX call
		var xmlhttp=new XMLHttpRequest();
		
		xmlhttp.open("GET","includes/functions_meleti_kelyfos.php?getzonedapeda=1&page="+page ,true);
		xmlhttp.send();
		
		xmlhttp.onreadystatechange=function()  {
		if (xmlhttp.readyState==4 && xmlhttp.status==200) {
			document.getElementById("zone_dapeda_table").innerHTML=xmlhttp.responseText;
	...
	function zone_dapeda_getzones(type,zone_id){
	type = typeof type !== 'undefined' ? type : 0;
	var prefix = "editzone_dapeda_";
	...
	var prefix = "editzone_dapeda_";
		...
		document.getElementById(prefix+"u_id").selectedIndex = <?php echo $dap_u_id;?>;
		zone_dapeda_u();
		...
		if(id!=0){
			var link = "includes/functions_meleti_general.php?get_iddata=1&table=meletes_zone_dapeda&id="+id;
			...
			if (xmlhttp.readyState==4 && xmlhttp.status==200) {
				var arr = JSON.parse(xmlhttp.responseText);	
					...
					zone_dapeda_getzones(0,arr["zone_id"]);
					document.getElementById(prefix+"type").selectedIndex = arr["type"];
					zone_dapeda_changetype();
					zone_dapeda_getzones(1,arr["mthx_id"]);
		...
		if(id!=0){
			button = '<button class="btn btn-warning" data-dismiss="modal" aria-hidden="true" onclick="submit_zone_dapeda('+id+','+page+');">Επεξεργασία</button>';
			edit_header = 'Επεξεργασία δαπέδου';
		}else{
			button = '<button class="btn btn-solar" data-dismiss="modal" aria-hidden="true" onclick="submit_zone_dapeda('+id+','+page+');">Προσθήκη</button>';
			edit_header = 'Προσθήκη δαπέδου';
		}
		document.getElementById("edit_button_zone_dapeda").innerHTML = button;
		document.getElementById("edit_header_zone_dapeda").innerHTML = edit_header;
		$("#modal_form_zone_dapeda").modal("show");
		modal_height();
	}
	
	//ΔΙΑΓΡΑΦΗ
	function formdel_zone_dapeda(id,page){
		var button = '<button class="btn btn-danger" data-dismiss="modal" aria-hidden="true" onclick="del_zone_dapeda('+id+','+page+');">Διαγραφή</button>';
		document.getElementById("del_button_zone_dapeda").innerHTML = button;
		$("#modal_del_zone_dapeda").modal("show");
	}
	
	//Υποβολή της φόρμας
	//Ανάλογα με το id γίνεται προσθήκη ή επεξεργασία
	function submit_zone_dapeda(id,page){
	var prefix = "editzone_dapeda_";
		...
		link += "&table=meletes_zone_dapeda";
		link += "&action="+action;
		link += "&id="+id;
		link += "&values="+zone_id+","+type+","+mthx_id+","+name+","+e+","+u+","+u_id+","+ap+","+ek+","+z+","+p+","+fhor_h+","+fhor_c+","+fov_h+","+fov_c+","+ffin_h+","+ffin_c;
		...
		xmlhttp.onreadystatechange=function()  {
		if (xmlhttp.readyState==4 && xmlhttp.status==200) {
			document.getElementById("zone_dapeda_info").innerHTML=xmlhttp.responseText;
			document.getElementById('wait').style.display="none";
			get_zone_dapeda(page);
			get_meleti_n();
		}}
	}
	
	//Διαγραφή
	function del_zone_dapeda(id,page){
		var link = "includes/functions_meleti_general.php?del_iddata=1&table=meletes_zone_dapeda&id="+id;
		document.getElementById('wait').style.display="inline";
		//AJAX call
		var xmlhttp=new XMLHttpRequest();
	...
	function zone_dapeda_changetype(){
	var prefix = "editzone_dapeda_";
	var type = document.getElementById(prefix+"type").value;
	...
	<!-- ###################### Κρυφό modal_form_zone_dapeda για εμφάνιση ###################### -->
	<div id="modal_form_zone_dapeda" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
	<div class="modal-dialog">
	<div class="modal-content">
		<div class="modal-header">
			<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
			<h6 id="myModalLabel"><span id="edit_header_zone_dapeda"></span></h6>
		</div>

		<div class="modal-body">
		<table class="table table-bordered table-condensed">
			<tr class="info"><td width=50%>Στοιχεία δαπέδου</td><td width=50%>Επιλογές επαφής</td></tr>
			<tr>
				<td>
					<div class="input-group">
					<span class="input-group-addon">
						<span class="tip-top" href="#" title="Θερμική ζώνη που ανήκει το δάπεδο"><i class="fa fa-cubes"></i> Ζώνη</span>
					</span>
					<select class="form-control input-sm" id="editzone_dapeda_zone_id">
						<option value=0></option>
					</select>
					</div>
				</td>
				<td rowspan="7">
				<span id="editzone_dapeda_help"></span>
					<div id="editzone_dapeda_help0" style="display:none;">
					<table class="table table-bordered table-condensed">
						<tr>
							<td>
								Βάθος έδρασης
							...
							<td>
								...
								<input class="form-control input-sm" type="text" id="editzone_dapeda_z">
								</div>
							</td>
						</tr>
						<tr>
							...
							<td>
								...
								<input class="form-control input-sm" type="text" id="editzone_dapeda_p">
								</div>
							</td>
						</tr>
					</table>
					</div>
					<div id="editzone_dapeda_help1" style="display:none;">
					<table class="table table-bordered table-condensed">
						<tr>
							<td>
							<div class="input-group">
							...
							<select class="form-control input-sm" id="editzone_dapeda_f" onchange="zone_dapeda_f();">
						...
						<tr><td>Fhor_h</td><td><input class="form-control input-sm" type="text" id="editzone_dapeda_fhor_h"></td></tr>
						<tr><td>Fhor_c</td><td><input class="form-control input-sm" type="text" id="editzone_dapeda_fhor_c"></td></tr>
						<tr><td>Fov_h</td><td><input class="form-control input-sm" type="text" id="editzone_dapeda_fov_h"></td></tr>
						<tr><td>Fov_c</td><td><input class="form-control input-sm" type="text" id="editzone_dapeda_fov_c"></td></tr>
						<tr><td>Ffin_h</td><td><input class="form-control input-sm" type="text" id="editzone_dapeda_ffin_h"></td></tr>
						<tr><td>Ffin_c</td><td><input class="form-control input-sm" type="text" id="editzone_dapeda_ffin_c"></td></tr>
					</table>
					</div>
				</td>
			</tr>
			<tr>
				<td>
					<div class="input-group">
					<span class="input-group-addon">
						<span class="tip-top" href="#" title="Τύπος επαφής δαπέδου<br/>Σε τμήμα κτιρίου επιλέγεται: σε αέρα και U=U/2"><i class="fa fa-braille"></i> Τύπος</span>
					</span>
					<select class="form-control input-sm" id="editzone_dapeda_type" onchange="zone_dapeda_changetype();">
						<option value=0>Επί εδάφους</option>
						<option value=1>Σε ΜΘΧ/Ηλιακό χώρο (διαχ. επιφάνεια)</option>
						<option value=2>Σε πυλωτή</option>
					</select>
					</div>
				</td>
			</tr>
			<tr>
				<td>
					<div class="input-group">
					<span class="input-group-addon">
						<span class="tip-top" href="#" title="Ο ΜΘΧ/Ηλιακός χώρος που έρχεται σε επαφή το δάπεδο"><i class="fa fa-cube"></i> ΜΘΧ</span>
					</span>
					<select class="form-control input-sm" id="editzone_dapeda_mthx_id">
						<option value=0></option>
					</select>
					</div>
				</td>
			...
			<tr>
				<td>
					...
					<input class="form-control input-sm" type="text" id="editzone_dapeda_name">
					</div>
				</td>
			</tr>
			<tr>
				<td>
					...
					<input class="form-control input-sm" type="text" id="editzone_dapeda_e">
			...
			<tr>
				...
				<div class="row">
					<div class="col-md-5">
						...
						<input class="form-control input-sm" type="text" id="editzone_dapeda_u">
					...
					<div class="col-md-6">
						...
						<select class="form-control input-sm" id="editzone_dapeda_u_id" onchange="zone_dapeda_u();">
			...
			<tr>
				...
				<div class="row">
					<div class="col-md-6">
						...
						<select class="form-control input-sm" id="editzone_dapeda_ap">
							<?php
					...
					<div class="col-md-6">
						...
						<select class="form-control input-sm" id="editzone_dapeda_ek">
							<?php
	...
	<div class="modal-footer">	
		<span id="edit_button_zone_dapeda"></span>
...
<!-- ###################### Κρυφό modal_del_zone_dapeda για εμφάνιση ###################### -->
<div id="modal_del_zone_dapeda" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
<div class="modal-dialog">
<div class="modal-content">
	<div class="modal-header">
		<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
		<h6 id="myModalLabel">Διαγραφή δαπέδου</h6>
	</div>
	...
	<div class="modal-footer">	
		<span id="del_button_zone_dapeda"></span>
		<button class="btn btn-default" data-dismiss="modal" aria-hidden="true">Άκυρο</button>
	</div>
</div>
</div>
