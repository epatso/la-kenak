...
function create_user_meletes($page=1,$like=""){
	...
	if($like!=""){
		$where=array(
			"AND"=>array("user_id"=>$_SESSION['user_id']),
	...
	$count_meletes=$database->count($tb,$where);
...
function create_library_help(){
	...
	if(!isset($database)){$database = new medoo(DB_NAME);}
	$tb = "vivliothiki_help";
	$col = "*";
...
function create_library_materials($category=0,$subcategory=0,$page=1,$like=""){
	...
	if($count!=0){
		$txt .= "Αποτελέσματα από ".$count_start." έως ".$count_end." σε σύνολο ".$count." υλικών ΤΟΤΕΕ.";
	}else{
