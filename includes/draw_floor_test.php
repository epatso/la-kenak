...
if (isset($_GET['floor'])){$floor=$_GET["floor"];}
	...
	foreach ($data_wall as $wall){
		...
		if($wall["g_type"]==0){
			...
			if($wall_g>=0 AND $wall_g<90){
				$x2=$current_x+$wall_l* cos(deg2rad($wall_g));
				$y2=$current_y+$wall_l* sin(deg2rad($wall_g));
			}
			if($wall_g>=90 AND $wall_g<180){
				$x2=$current_x-$wall_l* sin(deg2rad($wall_g-90));
				$y2=$current_y+$wall_l* cos(deg2rad($wall_g-90));
			}
			if($wall_g>=180 AND $wall_g<270){
				$x2=$current_x-$wall_l* cos(deg2rad($wall_g-180));
				$y2=$current_y-$wall_l* sin(deg2rad($wall_g-180));
			}
			if($wall_g>=270 AND $wall_g<360){
				$x2=$current_x+$wall_l* sin(deg2rad($wall_g-270));
				$y2=$current_y-$wall_l* cos(deg2rad($wall_g-270));
			}
	...
	$image_x=$x_min+$x_max+$scale*4 + 150;
	...
	//ΓΡΑΜΜΗ ΔΙΑΧΩΡΙΣΜΟΥ
	imageline($image, $image_x-160, 200, $image_x-160, 280+40*$count, $grey);
	//Πίνακας ορίζοντα
	...
	foreach ($data_wall as $wall){
		...
		if($wall["g_type"]==0){
			...
			if($wall_g>=0 AND $wall_g<90){
				$x2=$current_x+$wall_l* cos(deg2rad($wall_g));
				$y2=$current_y+$wall_l* sin(deg2rad($wall_g));
				$text_x = $current_x +($wall_l/2) * cos(deg2rad($wall_g));
				$text_y = $current_y +($wall_l/2)* sin(deg2rad($wall_g));
					//Ορίζοντας
					if($fhor[0]!=0){
						$x1_hor=$x1 + ($fhor[0]*$scale) * sin(deg2rad($wall_g));
						$y1_hor=$y1 - ($fhor[0]*$scale) * cos(deg2rad($wall_g));
						$x2_hor=$x2 + ($fhor[0]*$scale) * sin(deg2rad($wall_g));
						$y2_hor=$y2 - ($fhor[0]*$scale) * cos(deg2rad($wall_g));
					}
			}
			if($wall_g>=90 AND $wall_g<180){
				$x2=$current_x-$wall_l* sin(deg2rad($wall_g-90));
				$y2=$current_y+$wall_l* cos(deg2rad($wall_g-90));
				$text_x = $current_x -($wall_l/2) * sin(deg2rad($wall_g-90));
				$text_y = $current_y +($wall_l/2)* cos(deg2rad($wall_g-90));
					//Ορίζοντας
					if($fhor[0]!=0){
						$x1_hor=$x1 + ($fhor[0]*$scale) * cos(deg2rad($wall_g-90));
						$y1_hor=$y1 + ($fhor[0]*$scale) * sin(deg2rad($wall_g-90));
						$x2_hor=$x2 + ($fhor[0]*$scale) * cos(deg2rad($wall_g-90));
						$y2_hor=$y2 + ($fhor[0]*$scale) * sin(deg2rad($wall_g-90));
					}
			}
			if($wall_g>=180 AND $wall_g<270){
				$x2=$current_x-$wall_l* cos(deg2rad($wall_g-180));
				$y2=$current_y-$wall_l* sin(deg2rad($wall_g-180));
				$text_x = $current_x -($wall_l/2) * cos(deg2rad($wall_g-180));
				$text_y = $current_y -($wall_l/2)* sin(deg2rad($wall_g-180));
					//Ορίζοντας
					if($fhor[0]!=0){
						$x1_hor=$x1 - ($fhor[0]*$scale) * sin(deg2rad($wall_g-180));
						$y1_hor=$y1 + ($fhor[0]*$scale) * cos(deg2rad($wall_g-180));
						$x2_hor=$x2 - ($fhor[0]*$scale) * sin(deg2rad($wall_g-180));
						$y2_hor=$y2 + ($fhor[0]*$scale) * cos(deg2rad($wall_g-180));
					}
			}
			if($wall_g>=270 AND $wall_g<360){
				$x2=$current_x+$wall_l* sin(deg2rad($wall_g-270));
				$y2=$current_y-$wall_l* cos(deg2rad($wall_g-270));
				$text_x = $current_x +($wall_l/2) * sin(deg2rad($wall_g-270));
				$text_y = $current_y -($wall_l/2)* cos(deg2rad($wall_g-270));
					//Ορίζοντας
					if($fhor[0]!=0){
						$x1_hor=$x1 - ($fhor[0]*$scale) * cos(deg2rad($wall_g-270));
						$y1_hor=$y1 - ($fhor[0]*$scale) * sin(deg2rad($wall_g-270));
						$x2_hor=$x2 - ($fhor[0]*$scale) * cos(deg2rad($wall_g-270));
						$y2_hor=$y2 - ($fhor[0]*$scale) * sin(deg2rad($wall_g-270));
					}
			}
		...
		foreach($data_win as $window){
			...
			if($wall_g>=0 AND $wall_g<90){
				$win_x1=$leftside_x-$win_apoar* cos(deg2rad($wall_g));
				$win_y1=$leftside_y-$win_apoar* sin(deg2rad($wall_g));
				$win_x2=$win_x1-$win_l* cos(deg2rad($wall_g));
				$win_y2=$win_y1-$win_l* sin(deg2rad($wall_g));
				$text_win_x = $win_x1-($win_l/2)* cos(deg2rad($wall_g));
				$text_win_y = $win_y1-($win_l/2)* sin(deg2rad($wall_g));
			}
			if($wall_g>=90 AND $wall_g<180){
				$win_x1=$leftside_x+$win_apoar* sin(deg2rad($wall_g-90));
				$win_y1=$leftside_y-$win_apoar* cos(deg2rad($wall_g-90));
				$win_x2=$win_x1+$win_l* sin(deg2rad($wall_g-90));
				$win_y2=$win_y1-$win_l* cos(deg2rad($wall_g-90));
				$text_win_x = $win_x1+($win_l/2)* sin(deg2rad($wall_g-90));
				$text_win_y = $win_y1-($win_l/2)* cos(deg2rad($wall_g-90));
			}
			if($wall_g>=180 AND $wall_g<270){
				...
				$text_win_x = $win_x1+($win_l/2)* cos(deg2rad($wall_g-180));
				$text_win_y = $win_y1+($win_l/2)* sin(deg2rad($wall_g-180));
			}
			if($wall_g>=270 AND $wall_g<360){
				$win_x1=$leftside_x-$win_apoar* sin(deg2rad($wall_g-270));
				$win_y1=$leftside_y+$win_apoar* cos(deg2rad($wall_g-270));
				$win_x2=$win_x1-$win_l* sin(deg2rad($wall_g-270));
				$win_y2=$win_y1+$win_l* cos(deg2rad($wall_g-270));
				$text_win_x = $win_x1-($win_l/2)* sin(deg2rad($wall_g-270));
				$text_win_y = $win_y1+($win_l/2)* cos(deg2rad($wall_g-270));
			}
